...
public class ShopPeopleAdpater extends MyBaseAdapter {
	...
	static class ViewHolder {
		private ImageView gridview_people_image;
		private ImageView gridview_image_bg;
		
	}
	
	public ShopPeopleAdpater(Context context, List<? extends Object> list) {
		super(context, list);
...
//		String timeperiod= (String) getList().get(position);
		if (convertView == null) {
			...
			convertView = inflater.inflate(R.layout.item_shoppeople_gridview, null);
			holder.gridview_people_image = (ImageView) convertView.findViewById(R.id.gridview_people_image);
		...
		Bitmap  bitmaps = Bitmap.createScaledBitmap(bitmap, FileUtils.dip2px(context, 70), FileUtils.dip2px(context, 70), true);
		BitmapDrawable bd=new BitmapDrawable(bitmaps);
		holder.gridview_people_image.setBackgroundDrawable(bd);
